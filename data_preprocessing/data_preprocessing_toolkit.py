...
class DataPreprocessingToolkit(object):
    ...
    def add_length_of_stay(df):
        ...
        ########################
        res = df.copy()
        res[['date_from', 'date_to']] = res[['date_from', 'date_to']].apply(pd.to_datetime)
        res['length_of_stay'] = (res['date_to'] - res['date_from']).dt.days
        return res
    ...
    def add_book_to_arrival(df):
        ...
        ########################
        res = df.copy()
        res[['date_from', 'booking_date']] = res[['date_from', 'booking_date']].apply(pd.to_datetime)
        res['book_to_arrival'] = (res['date_from'] - res['booking_date']).dt.days
        return res
    ...
    def add_weekend_stay(df):
        ...
        dt = (df["date_from"] - df["date_to"]).dt.days
        df["weekend_stay"] = (((start >= 4) & (start != 6)) | (end >= 5) | ((end < start) & (start != 6)) | (dt >= 6)).apply(str)
        return df
    ...
    def add_night_price(df):
        ...
        ########################
        res = df.copy()
        res['night_price'] = np.round(res['accommodation_price'] / res['length_of_stay'] / res['n_rooms'], decimals=2)
        return res
    ...
    def sum_npeople(df):
        ...
        ########################
        res = df.copy()
        res['n_people'] = np.maximum(
            res['n_people'] + res['n_children_1'] + res['n_children_2'] + res['n_children_3'],
            1
        )
        return res
    ...
    def aggregate_group_reservations(self, df):
        ...
        group_df_1 = group_reservations[['group_id'] + self.sum_columns].groupby(['group_id']).sum()
        group_df_2 = group_reservations[['group_id'] + self.mean_columns].groupby(['group_id']).mean()
        group_df_3 = group_reservations[['group_id'] + self.mode_columns].groupby(['group_id']).agg(
            lambda x: x.value_counts().index[0])
        group_df_4 = group_reservations[['group_id'] + self.first_columns].groupby(['group_id']).first()
        group_reservations = group_df_1.merge(group_df_2, on='group_id').merge(group_df_3, on='group_id').merge(
            group_df_4, on='group_id')
        group_reservations.reset_index(drop=True, inplace=True)
        return pd.concat([non_group_reservations, group_reservations])
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        :rtype: pd.DataFrame
        """
        night_prices = df.groupby('room_group_id')['night_price'].mean().reset_index()
        night_prices.columns = ['room_group_id', 'night_price_v2']
        df = pd.merge(df, night_prices, on=['room_group_id'], how='left')
        df['room_segment'] = df['night_price_v2'].apply(
            lambda x: self.map_value_to_bucket(x, self.room_segment_buckets)
        )
        return df.drop(columns=['night_price_v2'])
